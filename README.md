...
El protocolo de comunicación consiste en paquetes que tienen tipos de mensajes (como Bet, Success, NO_MORE_BETS, etc). El tipo de mensaje ocupa exactamente un byte y siempre se pone al inicio del paquete, de esta manera tanto los clientes como el servidor saben el tipo de mensaje que están recibiendo por el socket. Como cada tipo de mensaje tiene un formato específico, una vez que se conoce el tipo de mensaje, el servidor o un cliente sabe como parsearlo (siempre y cuando sea un tipo de mensaje que debe handlear).
Los formatos para los mensajes son:
SUCCESS | valor tipo de mensaje: 0 (1 byte), no tiene payload
FAIL | valor tipo de mensaje: 1 (1 byte), no tiene payload
BETS | valor tipo de mensaje: 2 (1 byte)
header con campos: agency_size es el tamaño en bytes del campo agency (1 byte), agency es el string del id de la agencia (agency_size byte), amount_bets_size es el tamaño en bytes del campo amount_bets (1 byte) y amount_bets es el string de la cantidad de apuestas que lleva el mensaje (amount_bets_size byte)
payload con campos: name, lastname, document, birthday y number. Todos acompañados por su largo en bytes para saber cuántos bytes hay que leer por campo.
NO_MORE_BETS | valor tipo de mensaje: 3 (1 byte), su payload es el id del cliente con su tamaño en bytes.
CONSULT_WINNER | valor tipo de mensaje: 4 (1 byte), su payload es el id del cliente con su tamaño en bytes.
WINNERS | valor tipo del mensaje: 5 (1 byte), su payload es la cantidad de ganadores de la agencia y el tamaño en bytes.

Los clientes comienzan la conexión con el servidor, envían los BETS para que el servidor cargue las apuestas, envían NO_MORE_BETS para notificar que ya no hay más apuestas por envíar y por último CONSULT_WINNERS para pedir a los ganadores. Por su parte el servidor acepta a los clientes, les manda SUCCESS por cada mensaje de BETS procesado y con las apuesta cargadas, y por ultimo, cuando recibe CONSULT_WINNERS envía WINNERS con los ganadores a cada cliente.

### Mecanismos de sincronización utilizados

Para la concurrencia se implementó al servidor para que espere por conexiones nuevas. Por cada conexión levantaba un proceso para que sea el encargado de comunicarse con el cliente y de procesar sus mensajes BETS. Para eso se uso una Queue que usaba el proceso principal del servidor para escuchar al resto de procesos, que le advertían cuando un cliente había terminado de envíar apuestas, una serie de Pipes, uno por cada proceso creado, para que el proceso principal -una vez que tuviese a los ganadores de cada agencia- les mandase los resultados de las apuestas a cada proceso encargado de comunicarse con el cliente. Por último, se utilizó un Lock() para controlar el acceso al archivo de las apuestas, tanto para store_bets() (y tener exclusión mutua en escritura) como para load_bets() (exclusión mutua para lectura).
